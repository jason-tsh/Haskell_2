...
eval vars (ToNum x) = case x of
                        Get var -> case lookup3 var vars of
                                     ...
                                     _ -> Right "No matching variables"
                        _ -> eval vars x
eval vars (ToString x) = Left $ StrVal $ format $ case eval vars x of
                                                    Left val -> show val
                                                    Right _ -> "**Invalid/ out-of-scope expression**"
...
lookup3 name Leaf =  Right "No matching results"
...
extract name Leaf =  Right "No matching results"--https://hackage.haskell.org/package/base-4.16.0.0/docs/src/GHC-List.html
